...
export default function AdviceBox(props) {
  ...
  const extractAnswer = (text) => {
    if (typeof text !== "string") return text;
    let answerText = text;
    const idx = answerText.lastIndexOf("Adv_isory:");
  ...
  const detectSource = (text) => {
    ...
    if (
      ...
      return "AI"; // fallback if it’s not from paddy collections
    } else {
                ...
                if (cachedBlob) {
                  ...
                  if (type === "audio") {
                    ...
                    if (audioRes?.audio_url) {
                      const response = await fetch(`http://localhost:8001/outputs/${audioRes.audio_url.replace("./outputs/", "")}`);
                    ...
                    if (audioRes?.audio_file) {
                      const response = await fetch(`http://localhost:8001/azure_audios/${audioRes.audio_file.split("/").pop()}`);
                      const blobData = await response.blob();
                      await storeAudio(key, blobData);
                      audioUrl = URL.createObjectURL(blobData);
                    }
  ...
  return (
    <div className="advice-box">
                ...
                {dialogueLines.map((line, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <b>{line.speaker}:</b>{" Loading conversation..."}
      ...
      >
        <button onClick={() => setViewMode("paragraph")}>Paragraph</button>
        <button onClick={() => setViewMode("conversation")}>Conversation</button>
        {/* Buttons to switch */}
        <button onClick={() => setViewMode("audio")}>Audio</button>
